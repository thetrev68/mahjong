...
export class Card {
  ...
  generateHand(handDescription, numTiles) {
    ...
    outerLoop: for (const group of this.validHandGroups) {
  ...
  validateHand14(hand) {
    ...
    const test = hand.getAllTilesIncludingExposures
      ? hand.getAllTilesIncludingExposures()
      : hand.getTileArray();
  ...
  rankHand(hand, rankInfo, validHand) {
    ...
    for (let minNum = start; minNum <= end; minNum += delta) {
      ...
      for (const vsuitArray of permArray) {
        ...
        const componentInfoArray = this.rankFormComponents(
          hand,
          minNum,
          validHand,
          vsuitArray,
  ...
  rankMatchComp(test, minNum, comp, vsuitArray) {
    ...
    if (
      test.length === comp.count &&
      matchInfo.tileArray.length === comp.count
    ) {
      // Perfect match
  ...
  rankFormComponents(hand, minNum, validHand, vsuitArray) {
    ...
    for (const tileSet of hand.exposedTileSetArray) {
      ...
      for (const componentInfo of componentInfoArray) {
        ...
        const matchInfo = this.rankMatchComp(
          tileSet.tileArray,
          minNum,
    ...
    for (const joker of remHiddenJokers) {
      ...
      for (const componentInfo of remCompInfo) {
        const delta =
          componentInfo.component.count - componentInfo.tileArray.length;
