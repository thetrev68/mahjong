...
export class AIEngine {
  ...
  getTileRecommendations(handData, minDiscardableOverride = null) {
    ...
    const sortedRankCardHands = [...rankCardHands].sort(
      (a, b) => b.rank - a.rank,
    );
    ...
    const minDiscardable =
      minDiscardableOverride !== null
        ? minDiscardableOverride
        : this.config.minDiscardable;
    ...
    for (let tryCount = preferredCount; tryCount >= 1; tryCount--) {
      ...
      const discardableCount = this.countDiscardableTiles(
        handTiles,
        consideredPatterns,
      );

      debugPrint(
        `Trying ${tryCount} hand(s) alive: ${discardableCount} discardable tiles (need minimum ${minDiscardable})`,
      );
      ...
      if (discardableCount >= minDiscardable) {
        ...
        debugPrint(
          `Keeping ${tryCount} hand(s) alive with ${discardableCount} discard options`,
        );
        break;
      }
    ...
    for (const tile of handTiles) {
      ...
      if (tile.suit === SUIT.JOKER || tile.suit === SUIT.BLANK) {
        ...
        if (need && need.needed > 0) {
          ...
          recommendation = TILE_RECOMMENDATION.KEEP;
          debugPrint(
            `  ${tile.getText()}: KEEP (need.needed=${need.needed} before decrement)`,
          );
          need.needed--; // Decrement so next instance might be DISCARD
        } else {
          debugPrint(
            `  ${tile.getText()}: DISCARD (need=${need ? `${need.needed}/${need.have}` : "not found"})`,
          );
        }
    ...
    recommendations.sort((a, b) => {
      const order = {
        [TILE_RECOMMENDATION.KEEP]: 0,
        [TILE_RECOMMENDATION.PASS]: 1,
        [TILE_RECOMMENDATION.DISCARD]: 2,
      };
  ...
  chooseDiscard(handData) {
    ...
    if (
      ...
      discardableRecommendations.length > 0
    ) {
  ...
  claimDiscard(tileThrown, playerThrowing, handData, ignoreRank = false) {
    ...
    const rankThresholdMet =
      ignoreRank || rankInfo.rank > this.config.exposureThreshold;
  ...
  charlestonPass(handData) {
    ...
    debugPrint(
      `[Charleston] Jokers in hand: ${recommendations.filter((r) => r.tile.suit === SUIT.JOKER).length}`,
    );
    debugPrint(
      `[Charleston] Blanks in hand: ${recommendations.filter((r) => r.tile.suit === SUIT.BLANK).length}`,
    );
    ...
    const validRecommendations = recommendations.filter(
      (r) => !r.tile.isJoker() && !r.tile.isBlank(),
    );

    debugPrint(
      `[Charleston] Valid recommendations after filtering: ${validRecommendations.length}`,
    );
    debugPrint(
      `[Charleston] Jokers in valid: ${validRecommendations.filter((r) => r.tile.suit === SUIT.JOKER).length}`,
    );
    debugPrint(
      `[Charleston] Blanks in valid: ${validRecommendations.filter((r) => r.tile.suit === SUIT.BLANK).length}`,
    );

    validRecommendations.sort((a, b) => {
      const order = {
        [TILE_RECOMMENDATION.DISCARD]: 0,
        [TILE_RECOMMENDATION.PASS]: 1,
        [TILE_RECOMMENDATION.KEEP]: 2,
      };
    ...
    for (let i = 0; i < 3; i++) {
      if (validRecommendations.length > i) {
        ...
        debugPrint(
          `[Charleston] Passing tile ${i}: ${tile.getText()} (suit=${tile.suit})`,
        );
        pass.push(tile);
      }
    }
  ...
  courtesyPass(handData, maxCount) {
    ...
    for (
      let i = tileRecommendations.length - 1;
      i >= 0 && pass.length < maxCount;
      i--
    ) {
  ...
  exchangeTilesForJokers(handData, exposedJokerArray) {
    ...
    for (let i = 0; i < handTiles.length; i++) {
      ...
      for (const uniqueTile of exposedJokerArray) {
        if (
          tile.suit === uniqueTile.suit &&
          tile.number === uniqueTile.number
        ) {
          jokerFound = true;
          break;
        }
      ...
      const topHandsCount = Math.min(
        this.config.jokerTopHands,
        sortedRankCardHands.length,
      );
      for (let j = 0; j < topHandsCount; j++) {
        ...
        rank +=
          (copyHandRankArray[originalIndex].rank -
            rankCardHands[originalIndex].rank) *
          scale;
      }

      debugPrint(
        "exchangeTilesForJokers. Joker found for exchange. rank = " +
          rank +
          "\n",
      );
