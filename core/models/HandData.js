...
export class HandData {
   ...
   */
  getLength() {
    const exposedCount = this.exposures.reduce(
      (sum, exp) => sum + exp.tiles.length,
      0,
    );
  ...
  countTile(suit, number) {
    return this.tiles.filter((t) => t.suit === suit && t.number === number)
      .length;
  }
  ...
  static fromPhaserHand(phaserHand) {
    ...
    phaserHand.exposedTileSetArray.forEach((tileSet) => {
      ...
      exposure.tiles = tileSet.tileArray.map((tile) =>
