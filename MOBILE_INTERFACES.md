...
**UI_PROMPT** *(Special - requires callback)*
```javascript
{
...
class HandRenderer {
    ...
    constructor(container, gameController) {
        ...
        gameController.on('HAND_UPDATED', (data) => {
            if (data.player === 0) {  // Only render human player
...
class OpponentBar {
    ...
    update(handData) {
        ...
        this.container.querySelector('.tile-count').textContent = `${tileCount} tiles`;
    ...
    onTouchEnd(e) {
        ...
        if (duration < 300) {
            ...
            if (timeSinceLastTap < 500 && this.callbacks.onDoubleTap) {
                // Double-tap
                this.callbacks.onDoubleTap(e.changedTouches[0].clientX, e.changedTouches[0].clientY);
                this.lastTapTime = 0;  // Reset
            } else if (this.callbacks.onTap) {
                // Single tap
                this.callbacks.onTap(e.changedTouches[0].clientX, e.changedTouches[0].clientY);
...
export class MobileGameController {
    constructor() {
        ...
        this.opponentBars = [
            new OpponentBar(document.getElementById('opponent-1'), 1, this.gameController),
            new OpponentBar(document.getElementById('opponent-2'), 2, this.gameController),
            new OpponentBar(document.getElementById('opponent-3'), 3, this.gameController)
        ];
