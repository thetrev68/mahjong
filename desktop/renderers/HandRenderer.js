...
export class HandRenderer {
  ...
  syncAndRender(playerIndex, handData) {
    if (playerIndex < 0 || playerIndex > 3) {
      console.error(
        `HandRenderer.syncAndRender: Invalid playerIndex ${playerIndex}`,
      );
      return;
    }
    ...
    playerHand.hiddenTiles = handData.tiles
      .map((tileData) => {
        ...
        if (!phaserTile) {
          console.error(
            `HandRenderer.syncAndRender: Could not find/create Phaser tile for index ${tileData.index}`,
          );
        }
        return phaserTile;
      })
    ...
    playerHand.exposedSets = handData.exposures.map((exposure) => {
      return exposure.tiles
        .map((tileData) => {
          // Use getOrCreateTile to ensure tile is registered even if missing from map
          const phaserTile = this.tileManager.getOrCreateTile(tileData);
          if (!phaserTile) {
            console.error(
              `HandRenderer.syncAndRender: Could not find/create Phaser tile for exposure index ${tileData.index}`,
            );
          }
          return phaserTile;
        })
  ...
  showHand(playerIndex, forceFaceup = false) {
    if (playerIndex < 0 || playerIndex > 3) {
      console.error(
        `HandRenderer.showHand: Invalid playerIndex ${playerIndex}`,
      );
      return;
    }
    ...
    debugPrint(
      "HandRenderer.showHand called. playerIndex:",
      playerIndex,
      "forceFaceup:",
      forceFaceup,
    );
  ...
  renderHiddenTiles(playerIndex, playerInfo, playerHand, exposed) {
    ...
    const pos = this.calculateHiddenTilePositions(
      playerInfo,
      hiddenTiles.length,
    );
