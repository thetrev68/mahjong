...
export class DealingAnimationSequencer extends AnimationSequencer {
  ...
  async animateDeal(data = {}) {
    ...
    if (!sequence.length) {
      console.warn(
        "DealingAnimationSequencer: Empty sequence, skipping animation",
      );
    ...
    await this.executeSequence(
      sequence.map(
        (step, index) => () => this.dealBatch(step, index, sequence.length),
      ),
    );
  }
  ...
  async dealBatch(step, batchIndex, totalBatches) {
    const playerIndex =
      typeof step.player === "number" ? step.player : PLAYER.BOTTOM;
    ...
    if (stagedHand) {
      ...
      this.handRenderer.syncAndRender(
        playerIndex,
        this.gameController.players[playerIndex].hand,
      );
    }
  ...
  async dealSingleTile(
    ...
    if (!phaserTile) {
      debugError(
        `Could not find Phaser Tile for index ${tileData.index} during dealing`,
      );
      return;
    }
  ...
  onSequenceComplete() {
    ...
    if (
      ...
      typeof this.pendingHumanGlowTile.addGlowEffect === "function"
    ) {
    ...
    if (
      this.scene &&
      typeof this.scene.handleDealAnimationComplete === "function"
    ) {
