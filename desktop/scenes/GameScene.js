...
class GameScene extends Phaser.Scene {
  ...
  async create() {
    ...
    await this.gameController.init({
      ...
      settings: {
        ...
        skipCharleston:
          (window.settingsManager &&
            window.settingsManager.getSetting?.("skipCharleston", false)) ||
          false,
        trainingMode:
          (window.settingsManager &&
            window.settingsManager.getSetting?.("trainingMode", false)) ||
          false,
        trainingHand:
          (window.settingsManager &&
            window.settingsManager.getSetting?.("trainingHand", "")) ||
          "",
        trainingTileCount:
          (window.settingsManager &&
            window.settingsManager.getSetting?.("trainingTileCount", 13)) ||
          13,
      },
    });
    ...
    window.addEventListener("settingsChanged", (event) => {
      ...
      if (this.gameController && this.gameController.settings) {
        ...
        if (newSettings.useBlankTiles !== undefined) {
          this.gameController.settings.useBlankTiles =
            newSettings.useBlankTiles;
        }
        if (newSettings.skipCharleston !== undefined) {
          this.gameController.settings.skipCharleston =
            newSettings.skipCharleston;
        }
      }
    });
    ...
    this.wallGameBanner = this.add.text(
      WINDOW_WIDTH / 2,
      WINDOW_HEIGHT / 2,
      "Wall Game",
      {
  ...
  captureWallTiles() {
    if (
      !this.gTable ||
      !this.gTable.wall ||
      !Array.isArray(this.gTable.wall.tileArray)
    ) {
      return [];
    }
  ...
  getDefaultCommandBarPosition(bar, canvasBounds, boundsRect) {
    ...
    return this.getClampedCommandBarPosition(
      fallbackLeft,
      fallbackTop,
      bar,
      boundsRect,
  ...
  enableCommandBarDrag() {
    ...
    const onPointerMove = (event) => {
      ...
      const tentativeLeft =
        event.clientX -
        pointerOffsetX +
        (rect.width || bar.offsetWidth || 0) / 2;
  ...
  resize(_gameSize, _baseSize, _displaySize, _resolution) {
    ...
    const { left, top } = this.getDefaultCommandBarPosition(
      uicenterdiv,
      canvasBounds,
      boundsRect,
    );
