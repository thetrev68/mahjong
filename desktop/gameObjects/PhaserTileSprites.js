...
export class Tile {
  ...
  animate(x, y, angle, fixedDuration = null) {
    ...
    if (!this.sprite || !this.sprite.scene) {
      console.error(
        "Tile.animate: this.sprite is undefined, null, or destroyed for tile:",
        this.getText(),
      );
    ...
    const time =
      fixedDuration !== null ? fixedDuration : (distance * 1000) / speed;
    ...
    return this.withRaisedDepth(() => {
      ...
      if (
        Phaser.Math.Angle.Wrap(this.sprite.angle) !==
  ...
  addGlowEffect(scene, color = 0xff0000, intensity = 0.6, priority = 0) {
    ...
    // IMPORTANT: Must check glowEffect exists to distinguish "has glow" from "no glow with reset values"
    const hasRedHint =
      this.glowEffect && this.glowPriority === 0 && this.glowColor === 0xff0000;
    const hasBlueNew =
      this.glowEffect &&
      this.glowPriority === 10 &&
      this.glowColor === 0x1e3a8a;
    ...
    if (this.glowColor === "alternate") {
      // Alternating red/blue animation (2.4s cycle)
  ...
  updateGlowPosition() {
    ...
    const currentIntensity = this.glowAnimationData
      ? this.glowAnimationData.intensity
      : this.glowIntensity;
    ...
    if (this.glowColor === "alternate" && this.glowAnimationData) {
      ...
      if (phase < 0.45) {
        ...
        currentColor = 0x1e3a8a; // Dark blue (new-tile color)
      } else {
  ...
  removeGlowEffect() {
    ...
    if (this.glowEffect) {
      ...
      this.glowColor = null; // Reset color when removing glow
    }
  }
}
  ...
  receiveOrganizedTilesFromHomePage(homePageTiles) {
    return new Promise((resolve, reject) => {
      ...
      if (homePageTiles.length !== expectedTileCount) {
        console.error(
          "Wall.receiveOrganizedTilesFromHomePage: Invalid tile count. Expected " +
            expectedTileCount +
            ", got " +
      ...
      debugPrint(
        "Wall received " +
          homePageTiles.length +
          " tiles from HomePageTileManager.",
      );
      resolve();
    });
  }
...
export class Discards {
  ...
  getLastDiscardedTile() {
    ...
    return this.tileArray.length > 0
      ? this.tileArray[this.tileArray.length - 1]
      : null;
  }
  ...
  getTilePosition(index, totalCount = this.tileArray.length) {
    ...
    const tilesInRow = Math.max(
      Math.min(totalCount - row * tilesPerRow, tilesPerRow),
      1,
    );
