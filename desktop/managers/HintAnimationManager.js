...
export class HintAnimationManager {
  ...
  applyGlowToDiscardSuggestions(recommendations) {
    ...
    const discardRecs = recommendations.filter(
      (rec) =>
    ...
    discardRecs.forEach((rec, index) => {
      debugPrint(
        `Processing tile ${index + 1}: ${rec.tile.getText()} with recommendation ${rec.recommendation}`,
      );
    ...
    debugPrint(
      `Applied glow to ${this.glowedTiles.length} tiles out of ${discardRecs.length} discard tiles requested`,
    );
  ...
  findNextUnhighlightedTileInHand(handData, targetTile, highlightedTiles) {
    ...
    for (const tileData of hiddenTileDataArray) {
      ...
      if (
        tileData.suit === targetTile.suit &&
        tileData.number === targetTile.number
      ) {
  ...
  updateHintDisplay(rankCardHands, recommendations, consideredPatternCount) {
    ...
    for (let i = 0; i < Math.min(3, rankCardHands.length); i++) {
      ...
      const notConsideredLabel =
        !isConsidered && i > 0 ? " <em>(not considered)</em>" : "";

      // Render colorized pattern with matching
      const patternHtml = renderPatternVariation(
        rankHand,
        playerTiles,
        hiddenTiles,
      );
    ...
    const discardRecommendations = recommendations.filter(
      (rec) =>
