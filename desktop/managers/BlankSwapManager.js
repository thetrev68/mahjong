...
export class BlankSwapManager {
  ...
  isSwapPossible() {
    ...
    const selectableDiscards =
      this.discardPile?.getSelectableDiscards?.() || [];
  ...
  beginSwapFlow() {
    ...
    if (
      !this.discardPile ||
      (this.discardPile.getSelectableDiscards?.() || []).length === 0
    ) {
  ...
  commitSwap(blankTile, discardTile) {
    ...
    try {
      ...
      this.gameController.exchangeBlankWithDiscard(
        blankTileData,
        discardTileData,
      );
    } catch (error) {
