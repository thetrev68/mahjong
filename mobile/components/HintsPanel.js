...
export class HintsPanel {
  ...
  render() {
    ...
    this.toggleBtn.addEventListener("touchstart", this._onTouchStart, {
      passive: true,
    });
  ...
  computeAndRenderHints(handData) {
    ...
    try {
      ...
      const recommendationResult = this.aiEngine.getTileRecommendations(
        handData,
        1,
      );
      const consideredPatternCount =
        recommendationResult.consideredPatternCount;
      ...
      top3Patterns.forEach((rankHand, index) => {
        ...
        const notConsideredLabel =
          !isConsidered && index > 0 ? " <em>(not considered)</em>" : "";
  ...
  emitDiscardRecommendations(tiles = [], active = true) {
    if (
      !this.gameController ||
      typeof this.gameController.emit !== "function"
    ) {
      return;
    }
