...
export class PlayerRack {
  ...
  renderExposures(exposures, container) {
    ...
    if (!exposures || !Array.isArray(exposures)) {
      console.warn(
        "PlayerRack.renderExposures: Invalid exposures array provided",
      );
      return;
    }
    ...
    exposures.forEach((exposure, index) => {
      ...
      if (!exposure) {
        console.warn(
          `PlayerRack.renderExposures: Skipping null/undefined exposure at index ${index}`,
        );
        return;
      }
      ...
      if (tiles.length === 0) {
        console.warn(
          `PlayerRack.renderExposures: Empty or invalid tiles array for exposure at index ${index}`,
        );
      }
