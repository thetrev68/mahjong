...
export class DealingAnimationSequencer extends AnimationSequencer {
  ...
  async animateDeal(_dealSequence) {
    ...
    if (!humanPlayer || !this.handRenderer) {
      console.error(
        "DealingAnimationSequencer: Cannot animate deal - missing player or hand renderer",
      );
    ...
    if (
      !humanPlayer.hand ||
      !humanPlayer.hand.tiles ||
      humanPlayer.hand.tiles.length === 0
    ) {
      console.error(
        "DealingAnimationSequencer: Cannot animate deal - player hand is empty",
      );
      this.gameController.emit("DEALING_COMPLETE");
      return;
    }
  ...
  async renderHand(handData) {
    ...
    const tiles =
      this.handRenderer && Array.isArray(this.handRenderer.tiles)
