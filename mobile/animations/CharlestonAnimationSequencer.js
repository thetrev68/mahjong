...
export class CharlestonAnimationSequencer extends AnimationSequencer {
    ...
    async handleTilesReceived(data, receivedIndices) {
        ...
        if (currentHand && currentHand.tiles) {
            ...
            debugPrint("[CharlestonAnimationSequencer] Converted positions to tile IDs:", {
                receivedIndices,
                tileIds
            });
            this.receivedTileIndices = new Set(tileIds);
        } else {
            debugPrint("[CharlestonAnimationSequencer] No currentHandData, using receivedIndices directly");
    ...
    async animateSortWithGlow() {
        ...
        debugPrint(`[CharlestonAnimationSequencer] animateSortWithGlow: receivedTileIndices=${Array.from(this.receivedTileIndices).join(",")}, glowPositions=${glowPositions.join(",")}`);
