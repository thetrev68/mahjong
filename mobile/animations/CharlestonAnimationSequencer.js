...
export class CharlestonAnimationSequencer extends AnimationSequencer {
  ...
  async handleTilesReceived(data, receivedIndices) {
    ...
    if (currentHand && currentHand.tiles) {
      ...
      debugPrint(
        "[CharlestonAnimationSequencer] Converted positions to tile IDs:",
        {
          receivedIndices,
          tileIds,
        },
      ...
      debugPrint(
        "[CharlestonAnimationSequencer] No currentHandData, using receivedIndices directly",
      );
  ...
  async animateSortWithGlow() {
    ...
    if (!humanPlayer || !humanPlayer.hand) {
      console.warn(
        "CharlestonAnimationSequencer: Cannot sort - no human player hand",
      );
      return;
    }
    ...
    // Convert tile IDs (0-151) to positions in sorted hand
    const sortedTiles = handData.tiles;
    const glowPositions = [];
    this.receivedTileIndices.forEach((tileId) => {
      const position = sortedTiles.findIndex((t) => t?.index === tileId);
      if (position >= 0) {
        glowPositions.push(position);
      }
    });

    // Re-render with glow on received tiles (LAST)
    debugPrint(
      `[CharlestonAnimationSequencer] animateSortWithGlow: receivedTileIndices=${Array.from(this.receivedTileIndices).join(",")}, glowPositions=${glowPositions.join(",")}`,
    );
