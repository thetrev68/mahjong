...
export class MobileRenderer extends BaseAdapter {
  ...
  constructor(options = {}) {
    ...
    this.playerRack = new PlayerRack(
      document.getElementById("player-rack-container"),
    );
  ...
  registerEventListeners() {
    this.registerEventHandlers({
      ...
      COURTESY_VOTE: (data) =>
        this.updateStatus(
          `Player ${data.player} voted ${data.vote} for courtesy pass`,
        ),
  ...
  setupButtonListeners() {
    ...
    if (this.mahjongButton)
      this.mahjongButton.addEventListener("click", () => this.declareMahjong());
  ...
  onSortClicked() {
    if (
      !this.latestHandSnapshot ||
      !Array.isArray(this.latestHandSnapshot.tiles)
    ) {
      return;
    }
    ...
    const sortedHand =
      typeof this.latestHandSnapshot.clone === "function"
  ...
  canDrawTile() {
    ...
    return (
      this.gameController.currentPlayer === HUMAN_PLAYER &&
  ...
  canSwapJoker() {
    ...
    const hasMatchingTiles = humanPlayer.hand.tiles.some(
  ...
  canDeclareMahjong() {
    ...
    const validationResult = this.gameController.cardValidator.validateHand(
      tiles,
      allHidden,
    );
  ...
  updateBlankSwapButton() {
    ...
    const hasBlankTiles =
      player?.hand?.tiles?.some((tile) => tile.isBlank()) ?? false;
    const hasDiscards = this.gameController.discards.length > 0;
    const isValidState =
      this.gameController.state === STATE.LOOP_CHOOSE_DISCARD;
    const blankRuleEnabled =
      this.gameController.settings?.useBlankTiles ?? false;

    swapBtn.style.display =
      hasBlankTiles && hasDiscards && isValidState && blankRuleEnabled
        ? "flex"
        : "none";
  }
  ...
  updateMahjongButton() {
    ...
    mahjongBtn.style.display =
      canDeclare && isValidState && isHumanTurn ? "flex" : "none";
  }
  ...
  async handleBlankSwap() {
    ...
    if (blankTiles.length === 0) {
      this.showAlert(
        "No Blank Tiles",
        "You don't have any blank tiles to swap.",
      );
      return;
    }
    ...
    try {
      ...
      if (success) {
        this.showAlert(
          "Swap Successful",
          `Blank exchanged for ${discardTile.getText()}`,
        );
  ...
  showDiscardSelection() {
    return new Promise((resolve) => {
      ...
      if (availableDiscards.length === 0) {
        this.showAlert(
          "No Valid Discards",
          "No tiles available to swap (jokers cannot be swapped).",
        );
        resolve(null);
        return;
      }
  ...
  onGameStarted() {
    ...
    if (this.homePageTiles) {
      this.homePageAnimationPromise = this.homePageTiles
        .animateStart()
        .catch((err) => {
  ...
  onStateChanged(data) {
    ...
    if (sortBtn) {
      const isLoopState =
        data.newState >= STATE.LOOP_PICK_FROM_WALL &&
        data.newState <= STATE.LOOP_EXPOSE_TILES_COMPLETE;
  ...
  onTurnChanged(data) {
    const currentPlayer =
      data?.currentPlayer ?? this.gameController.currentPlayer;
    ...
    if (currentPlayer === HUMAN_PLAYER) {
      ...
      const bar = this.opponentBars.find(
        (ob) => ob.playerIndex === currentPlayer,
      );
  ...
  async onTileDiscarded(data) {
    ...
    try {
      ...
      try {
        ...
        console.error(
          "Failed to apply fallback discard update:",
          fallbackError,
        );
      }
    }
  ...
  onCharlestonPass(data) {
    ...
    if (!data || !this.charlestonSequencer) {
      console.warn(
        "MobileRenderer: Missing required data for Charleston pass animation",
      );
      return;
    }
    ...
    try {
      ...
      const passingIndices = Array.from(
        this.selectionManager.getSelectedTileIndices(this.latestHandSnapshot),
      );
  ...
  onTilesReceived(data) {
    ...
    debugPrint(
      "[MobileRenderer] onTilesReceived: Skipping Charleston animation (HandEventCoordinator handles glow)",
    );
    return;
  }
  ...
  _findReceivedTileIndices(receivedTiles) {
    ...
    for (let i = 0; i < handTiles.length; i++) {
      ...
      if (
        handTile?.index !== undefined &&
        receivedTileIds.has(handTile.index)
      ) {
        indices.push(i);
      }
    }
  ...
  async onTileClaimed(data) {
    ...
    if (!this.handRenderer || !this.animationController || !data?.tile) {
      console.warn(
        "MobileRenderer: Missing required components for claim animation",
      );
      this.discardPile.removeLatestDiscard();
      return;
    }
  ...
  onTilesExposed(data) {
    ...
    if (
      ...
      const exposedTileIndices = data.tiles
        .map((t) => t.index)
      ...
      setTimeout(() => {
        const exposureTiles =
          this.playerRack.container.querySelectorAll(".exposure-tile");
  ...
  handleUIPrompt(data) {
    ...
    try {
      ...
      if (this.pendingPrompt) {
        console.warn(
          "MobileRenderer: New prompt received while previous prompt pending. Clearing previous prompt state.",
        );
        this.resetHandSelection();
        this.hidePrompt();
        this.pendingPrompt = null;
      ...
      switch (data.promptType) {
        ...
        case "CLAIM_DISCARD": {
          ...
          const tileObj =
            promptTile instanceof TileData
              ? promptTile
              : promptTile
                ? TileData.fromJSON(promptTile)
                : null;
          this.showChoicePrompt({
        ...
        case "COURTESY_VOTE":
          this.showChoicePrompt({
            ...
            options: (data.options?.options || ["0", "1", "2", "3"]).map(
              (option) => ({
                label: option,
                value: option,
              }),
      ...
      debugPrint(
        `[MobileRenderer] Error in UI prompt handler: ${error.message}`,
      );
      ...
      if (data.callback) {
        try {
          ...
          console.error(
            "Failed to recover from UI prompt error via callback:",
            cbError,
          );
        }
      }
  ...
  onHandSelectionChange(
    selection = this.selectionManager.getSelectionState(
      this.latestHandSnapshot,
    ),
  ) {
  ...
  updateTileSelectionHint(
    ...
    this.setPromptHint(
      `Selected ${count}/${max}${min === max ? "" : ` (need at least ${min})`}`,
    );
  ...
  getFallbackTiles(count = 1) {
    ...
    return this.latestHandSnapshot.tiles
      .slice(0, count)
      .map((tile) => {
  ...
  updateActionButtonStateForGame(newState) {
    ...
    const isHumanTurn = this.gameController.currentPlayer === HUMAN_PLAYER;
    const isDiscardState =
      newState === STATE.LOOP_CHOOSE_DISCARD && isHumanTurn;
    ...
    if (isDiscardState && this.pendingPrompt?.type === "tile-selection") {
      ...
      const ready =
        selection.count >= (this.pendingPrompt.min || 1) &&
        selection.count <= (this.pendingPrompt.max || 1);
      this.updateActionButton({
        label: "Discard",
        onClick: () => this.confirmPendingSelection(),
        disabled: !ready,
