...
export class HandRenderer {
  ...
  renderWithGlow(handData, glowIndices) {
    ...
    const indices = Array.isArray(glowIndices)
      ? glowIndices
      : Array.from(glowIndices || []);
  ...
  render(handData) {
    ...
    this.renderExposures(
      Array.isArray(handData.exposures) ? handData.exposures : [],
    );
    this.clearTiles();
    ...
    if (this.selectionManager) {
      const currentSelection =
        this.selectionManager.getSelectionState(handData);
  ...
  clearTiles() {
    if (this.debug) {
      console.log(
        `[HandRenderer] Clearing ${this.tiles.length} tiles from DOM`,
      );
    }
    ...
    if (this.handContainer) {
      ...
      if (this.debug) {
        console.log(
          `[HandRenderer] DOM cleared, handContainer.children.length = ${this.handContainer.children.length}`,
        );
      }
    }
  }
