...
test.describe("Phase 5: Responsive Design & Component Polish", () => {
  ...
  viewports.forEach((viewport) => {
    test(`layout looks correct on ${viewport.name} (${viewport.width}x${viewport.height})`, async ({
      page,
    }) => {
      ...
      const gridColumns = await discardPile.evaluate(
        (el) =>
    ...
    test(`tiles are appropriately sized on ${viewport.name}`, async ({
      page,
    }) => {
      await page.setViewportSize({
        width: viewport.width,
  ...
  test("opponent bar highlights current turn with golden glow", async ({
    ...
    const hasGoldenBorder = await currentTurnBar.evaluate((el) => {
      ...
      return (
        style.borderColor.includes("255, 209, 102") ||
        style.borderColor.includes("ffd166")
      );
    });
  ...
  test("responsive breakpoints adjust tile sizes correctly", async ({
    page,
  }) => {
    const breakpoints = [
    ...
    for (const breakpoint of breakpoints) {
      ...
      expect(tileWidth).toBeGreaterThanOrEqual(
        breakpoint.expectedTileWidth - 3,
      );
  ...
  test("all UI elements fit within viewport without horizontal scroll", async ({
    page,
  }) => {
    const viewports = [
