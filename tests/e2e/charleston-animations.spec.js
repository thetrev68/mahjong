...
test.describe("Charleston Animations", () => {
  ...
  test("should animate Charleston pass right correctly", async ({ page }) => {
    ...
    await page.waitForSelector(".mobile-prompt:not(.hidden)", {
      timeout: 10000,
    });
    const promptText = await page
      .locator(".mobile-prompt__message")
      .textContent();
    ...
    await page.waitForFunction(
      () => {
        ...
        return Array.from(tiles).every(
          (tile) => !getComputedStyle(tile).transition.includes("transform"),
        );
      },
      { timeout: 1200 },
    );
  ...
  test("should handle Charleston phase 1 complete sequence", async ({
    page,
  }) => {
    // Start game
    await page.click("#new-game-btn");
  ...
  test("should animate all three directions with correct vectors", async ({
    page,
  }) => {
    // Start game
    await page.click("#new-game-btn");
