...
test.describe("JokerExchangeManager", () => {
  ...
  test("performExchange - Successfully exchanges tile for joker", async () => {
      ...
      botPlayer.hand.addExposure([new TileData(SUIT.BAMBOO, 1, 10), jokerTile], "Pung");
      ...
      expect(humanPlayer.hand.tiles.some(t => t.isSameTile(humanTile))).toBe(false);
      
      // Bot exposure should have the human tile
      expect(botPlayer.hand.exposures[0].tiles[1]).toBe(humanTile);
      
      // Events
      expect(gameController.events.some(e => e.event === "JOKER_SWAPPED")).toBe(true);
      expect(gameController.events.filter(e => e.event === "HAND_UPDATED")).toHaveLength(2);
  });
