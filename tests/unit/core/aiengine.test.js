...
async function createTestFixtures(difficulty = "medium") {
  const card = new Card();
  await card.init(2025);
...
test("AIEngine getDifficultyConfig - returns easy config", async () => {
  const { aiEngine } = await createTestFixtures("easy");
...
test("AIEngine getDifficultyConfig - returns medium config", async () => {
  const { aiEngine } = await createTestFixtures("medium");
...
test("AIEngine getDifficultyConfig - returns hard config", async () => {
  const { aiEngine } = await createTestFixtures("hard");
...
test("AIEngine getDifficultyConfig - defaults to medium for invalid difficulty", async () => {
  const { aiEngine } = await createTestFixtures("invalid");
  expect(aiEngine.config.discardRandomness).toBe(0.1);
});

test("AIEngine chooseDiscard - returns a TileData object", async () => {
  const { aiEngine } = await createTestFixtures();
...
test("AIEngine chooseDiscard - never discards jokers", async () => {
  const { aiEngine } = await createTestFixtures();
  const handData = new HandData();
...
test("AIEngine claimDiscard - returns valid playerOption", async () => {
  const { aiEngine } = await createTestFixtures();
  const handData = new HandData();
  ...
  expect([
    PLAYER_OPTION.MAHJONG,
    PLAYER_OPTION.DISCARD_TILE,
    PLAYER_OPTION.EXPOSE_TILES,
  ]).toContain(result.playerOption);
});

test("AIEngine charlestonPass - returns 3 tiles", async () => {
  const { aiEngine } = await createTestFixtures();
  const handData = new HandData();
...
test("AIEngine charlestonPass - never passes jokers", async () => {
  const { aiEngine } = await createTestFixtures();
  const handData = new HandData();
...
test("AIEngine charlestonPass - never passes blanks", async () => {
  const { aiEngine } = await createTestFixtures();
  const handData = new HandData();
...
test("AIEngine courtesyVote - returns a number between 0 and 3", async () => {
  const { aiEngine } = await createTestFixtures();
  const handData = new HandData();
...
test("AIEngine courtesyPass - returns up to maxCount tiles", async () => {
  const { aiEngine } = await createTestFixtures();
  const handData = new HandData();
...
test("AIEngine courtesyPass - never passes jokers or blanks", async () => {
  const { aiEngine } = await createTestFixtures();
  const handData = new HandData();
  ...
  expect(
    pass.every((tile) => tile.suit !== SUIT.JOKER && tile.suit !== SUIT.BLANK),
  ).toBe(true);
});

test("AIEngine exchangeTilesForJokers - returns object with correct properties", async () => {
  const { aiEngine } = await createTestFixtures();
  const handData = new HandData();
...
test("AIEngine exchangeTilesForJokers - returns false when no exposed jokers", async () => {
  const { aiEngine } = await createTestFixtures();
  const handData = new HandData();
...
test("AIEngine validateComponentForExposure - rejects components with less than 3 tiles", async () => {
  const { aiEngine } = await createTestFixtures();
  const compInfo = {
    component: { count: 2 },
    tileArray: [new TileData(SUIT.CRACK, 1, 0), new TileData(SUIT.CRACK, 1, 1)],
  };
...
test("AIEngine validateComponentForExposure - accepts valid pung (3 tiles)", async () => {
  const { aiEngine } = await createTestFixtures();
  const compInfo = {
    component: { count: 3 },
    tileArray: [
...
test("AIEngine validateComponentForExposure - accepts valid kong (4 tiles)", async () => {
  const { aiEngine } = await createTestFixtures();
  const compInfo = {
    component: { count: 4 },
    tileArray: [
