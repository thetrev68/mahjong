...
const handler = new TouchHandler(element, {
  onTap: (x, y) => {
    /* select tile at coords */
  },
  onDoubleTap: (x, y) => {
    /* discard tile */
  },
  onSwipeUp: (startY, endY) => {
    /* expose tiles */
  },
  onLongPress: (x, y) => {
    /* show menu */
...
test("mobile tile selection", async ({ page }) => {
  ...
  await expect(page.locator('.mobile-tile[data-index="0"]')).toHaveClass(
    /selected/,
  );
});
```
...
**Day 2:** 3. You: Implement Phase 1B (GameController) 4. Gemini Flash: Execute Phase 1C (move card/ folder)

**Day 3:** 5. You: Review Phase 1 deliverables 6. If approved, start Phase 2A (PhaserAdapter)
...
2. localStorage-based persistence with "mahjong\_" prefix to avoid conflicts
