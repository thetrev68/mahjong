...
test("tile animations play smoothly", async ({ page }) => {
  ...
  await page.waitForFunction(
    () => {
      const drawingTiles = document.querySelectorAll(
        ".mobile-tile.tile-drawing",
      );
...
export async function verifyOpponentBar(page, playerIndex, expectedData) {
  ...
  if (expectedData.name) {
    await expect(bar.locator(".opponent-name")).toContainText(
      expectedData.name,
    );
  }

  if (expectedData.tileCount !== undefined) {
    await expect(bar.locator(".tile-count")).toContainText(
      String(expectedData.tileCount),
    );
  }
