...
test.describe('Sprite Rendering', () => {
  test('tiles display as sprites, not text', async ({ page }) => {
    ...
    const textTiles = await page.locator('.tile:has-text("1B"), .tile:has-text("2C")').count();
  ...
  test('sprite positioning is correct', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      const pos = window.tileSprites.getSpritePosition({rank: 1, suit: 'BAM'});
